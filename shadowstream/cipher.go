...
// Hchacha20
type hchacha20key []byte

func (k hchacha20key) IVSize() int                       { return chacha20.HNonceSize }
func (k hchacha20key) Decrypter(iv []byte) cipher.Stream { return k.Encrypter(iv) }
func (k hchacha20key) Encrypter(iv []byte) cipher.Stream {
	ciph, err := chacha20.NewCipher(k, iv)
	if err != nil {
		panic(err) // should never happen
	}
...
func Hchacha20(key []byte) (Cipher, error) {
	if len(key) != chacha20.KeySize {
		return nil, KeySizeError(chacha20.KeySize)
	}
	return hchacha20key(key), nil
}
